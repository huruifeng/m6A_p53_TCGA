...
gene_list = {"SOX1":"ENSG00000181449"}
...
for cancer in ["lgg","gbm","brca","read"]:
    ...
    ####################################
    used_mut_expr = mis_mut_sample_expr

    sub_wt_df = samples_tumor.loc[:,wt_samples_expr]
    sub_mut_df = samples_tumor.loc[:, used_mut_expr]

    print("wt vs missense:", sub_wt_df.shape[1],sub_mut_df.shape[1])
    ...
    fout = open(cancer+"_mis_p_table.txt", "w")
    for gene_i in gene_list:
        ...
        print("wt vs mis:", len(wt_expr),len(mut_expr))

        total_n = len(wt_expr) + len(mut_expr)

        case_type = ['TP53_WT'] * len(wt_expr) + ["TP53_Mis"] * len(mut_expr)
        ...
        # ax.set(xlabel="Genes")
        # ax.set(title='Gene expression in TCGA - ' + cancer.upper())
        ...
        f.savefig(cancer + "_mis_"+gene_i+".pdf")
        plt.close()
    ...
    # ax.set_xticks([])
    # # ax.get_legend().remove()
    # f.savefig(cancer + "_mis_"+gene_i+"_legend.pdf")
    # plt.close()
