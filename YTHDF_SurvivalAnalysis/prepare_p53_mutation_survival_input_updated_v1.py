...
for cancer in cancer_ls:
    print("=========================================")
    sur_df = pd.read_csv("../TCGA/data/" + cancer + "/survival.tsv",index_col=0,header=0,sep="\t")
    sur_sample_name_ls = [x[:-1] for x in sur_df.index]
    sur_df.index = sur_sample_name_ls
    ...
    all_mut_sample_expr_ls = set([x for x in all_mut_samples if x in samples_tumor_ls])
    mis_mut_sample_expr_ls = set([x for x in mis_mut_samples if x in samples_tumor_ls])
    GR_mut_sample_expr_ls = set([x for x in GR_mut_samples if x in samples_tumor_ls])

    #######################################
    wt_sample_expr = samples_tumor.drop(all_mut_sample_expr_ls, axis=1)
    wt_samples = list(wt_sample_expr.columns)

    mut_samples = all_mut_sample_expr_ls

    wt_samples_w_sur = set([x for x in wt_samples if x in sur_sample_name_ls])
    mut_samples_w_sur = set([x for x in mut_samples if x in sur_sample_name_ls])

    ############################################
    sub_wt_df = samples_tumor.loc[gene_ensemble,wt_samples_w_sur]
    sub_mut_df = samples_tumor.loc[gene_ensemble, mut_samples_w_sur]
    ...
    with open("../TCGA/data/"+cancer+"/survival.tsv","r") as f:
        fp.write(f.readline()[:-1] + "\tTP53\tExpr\n")
        for line in f:
            line_ls = line.strip().split("\t")
            sample = line_ls[0][:-1]
            i = 0
